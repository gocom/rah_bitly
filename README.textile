...
To start using rah_bitly, you will first need to configure your *Bitly access token* and a "custom field":http://textpattern.net/wiki/index.php?title=custom_field you want to use to store the shortened links. You can find the options in Textpattern admin at *Admin* / *Preferences* and from the page *Bitly Integration* section.

* *Bitly access token* is your generic access token key. You can "get your own access token here":https://app.bitly.com/settings/api/.
* *Store in a custom field* is the custom field that will be used to store the shortened links.
...
Displaying Bitly links on your page templates and articles is easy and customizable due to the fact that a "custom field":http://textpattern.net/wiki/index.php?title=custom_field is used to store the shortened links.
...
The above snippet would display shortened Bitly link when placed to article context, or @No short link.@ message, if the article does not have short link.
